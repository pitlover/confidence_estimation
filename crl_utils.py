...
class History(object):
    ...
    # confidence update
    def confidence_update(self, data_idx, confidence, output):
        probs = torch.nn.functional.softmax(output, dim=1)
        confidence, _ = probs.max(dim=1)
        data_idx = data_idx.cpu().numpy()
    ...
    def get_target_margin(self, data_idx1, data_idx2):
        ...
        # print(cum_correctness1)
        # print("------------------------------")
        # print(self.max_correctness)
        # print(cum_correctness1.sum())
        acc = cum_correctness1.sum()/ (self.max_correctness * len(data_idx1))
        # print("ACC = ", acc)
        ...
        ## correctness i>j : 1, i=j : 0, i<j = 01
        ...
        return target, margin, acc, cum_correctness1
    ...
    def get_correctness(self):
        ## Normalize
        self.correctness = self.correctness_normalize(self.correctness)
