...
def calc_metrics(loader, label, label_onehot, model, criterion, args):
    acc, softmax, correct, logit, cls_loss, total_gt = get_metric_values(loader, model, criterion, args)
    # aurc, eaurc
    ...
    ece = calc_ece(softmax, total_gt, bins=15)
    # brier, nll
    # nll, brier = calc_nll_brier(softmax, logit, label, label_onehot)
    nll, brier = calc_nll_brier(softmax, logit, total_gt, label_onehot)

    E99, over_99 = calc_E99(softmax, correct, loader)
    return acc, aurc, eaurc, aupr, fpr, ece, nll, brier, E99, over_99, cls_loss

# E99
def calc_E99(softmax, correct, loader):
    softmax = np.array(softmax)
    correctness = np.array(correct)
    softmax_max = np.max(softmax, 1)

    over_99_idx = np.where(softmax_max >= 0.99)
    E99 = 1 - correctness[over_99_idx].sum() / len(over_99_idx[0])

    print("E99 {0:.2f} [{1}] ".format(E99 * 100, len(over_99_idx[0])))
    return E99, len(over_99_idx[0])
...
# Get softmax, logit
def get_metric_values(loader, model, criterion, args):
    model.eval()
    with torch.no_grad():
        total_loss = 0
        total_acc = 0
    ...
    return total_acc.item(), list_softmax, list_correct, list_logit, total_loss, total_gt
